...
import type { GraphData, Id } from "@/lib/types";
import { movieProjection, type PathResult } from "@/lib/graph";
...
const buildElements = (data: GraphData): ElementDefinition[] => {
  const nodes: ElementDefinition[] = Object.values(data.movies).map((m) => ({
    data: { id: m.id, label: m.name },
  }));
  const edges: ElementDefinition[] = movieProjection(data).map((e) => ({
    data: {
      id: `${e.source}~${e.target}`,
      source: e.source,
      target: e.target,
      label: e.actorIds
        .map((a) => data.actors[a]?.name)
        .filter(Boolean)
        .join(", "),
    },
  }));
...
export function MovieGraph({ data, path, onSelectMovie }: MovieGraphProps) {
